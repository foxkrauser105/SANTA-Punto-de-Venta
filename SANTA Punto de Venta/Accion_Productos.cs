...
namespace SANTA_Punto_de_Venta
{
    public partial class Accion_Productos : Form
    {
        ...
        {
            if (accion.Equals("Descuento") && !textBoxCodigo.Text.Equals(""))
            {
                try
                {
                    using (SqlConnection openCon = new SqlConnection(Properties.Settings.Default.SANTA_Connection))
                    {
                        openCon.Open();
                        SqlTransaction transaction = openCon.BeginTransaction();
                        SqlDataAdapter select = new SqlDataAdapter("SELECT p.nombre,         p.marca,           p.categoria, " +
                                                                   "       d.cantidadMinima, d.precioDescuento, d.status " +
                                                                   "FROM   productos p " + 
                                                                   "LEFT OUTER JOIN descuentos d " +
                                                                   "ON     p.id_producto = d.id_producto " +
                                                                   "WHERE  p.id_producto = '" + textBoxCodigo.Text + "'" + 
                                                                   "AND    p.status <> 0", openCon);
                        select.SelectCommand.Transaction = transaction;
                        ...
                        {
                            MessageBox.Show("El producto que ingresaste no existe o está en status inactivo. Prueba a usar el Buscador de Productos usando F9", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            textBoxCodigo.Focus();
                            textBoxNombre.Text = "";
                            textBoxMarca.Text = "";
                            textBoxCategoria.Text = "";
            ...
            if (buttonAplicar.Text.Equals("Agregar"))
            {
                ...
                else
                {
                    using (SqlConnection openCon = new SqlConnection(Properties.Settings.Default.SANTA_Connection))
                    {
                        openCon.Open();
                        SqlTransaction transaction = openCon.BeginTransaction();
                        SqlDataAdapter select = new SqlDataAdapter("SELECT count(*) " +
                                                                   "FROM   descuentos " +
                                                                   "WHERE  id_producto = '" + textBoxCodigo.Text + "'", openCon);
                        select.SelectCommand.Transaction = transaction;
                        DataTable dtProductos = new DataTable();
                        select.Fill(dtProductos);
