    ...
    public partial class Productos_Inactivos : Form
    {
        ...
        private void buttonReactivar_Click(object sender, EventArgs e)
        {
            ...
            if (con.ShowDialog(this) == DialogResult.Yes && con.textBoxContraseña.Text.Equals("hancock7"))
            {
                ...
                if (MessageBox.Show("¿Estás seguro de que quieres reactivar el producto '" + dataGridViewProductos.Rows[dataGridViewProductos.SelectedCells[0].RowIndex].Cells[1].Value.ToString() + "'?",
                    "Pregunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    using (SqlConnection openCon = new SqlConnection(Properties.Settings.Default.SANTA_Connection))
                    {
                        ...
                        try
                        {
                            string inactivate = "UPDATE productos SET status = 1 where id_producto = @id_producto";
                            using (SqlCommand queryUpdate = new SqlCommand(inactivate))
                            {
                                queryUpdate.Connection = openCon;
                                queryUpdate.Transaction = transaction;
                                queryUpdate.Parameters.Add("@id_producto", SqlDbType.VarChar).Value = dataGridViewProductos.Rows[dataGridViewProductos.SelectedCells[0].RowIndex].Cells[0].Value.ToString();
                        ...
                        catch (SqlException)
                        {
                            ...
                            MessageBox.Show("Ha habido un error. Verifica lo siguiente:\n\n- Verifica la conexión a la base de datos y prueba de nuevo\n- Verifica que el código de barras del producto a desactivar es correcto", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                }
            }
