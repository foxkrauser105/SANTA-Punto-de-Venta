...
namespace SANTA_Punto_de_Venta
{
    public partial class Editar_Producto : Form
    {
        ...
        //SANTADBContext santaDBContext = new SANTADBContext();
        ...
        private void textBoxCantidad_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            e.Handled = !(char.IsNumber(e.KeyChar) || char.IsControl(e.KeyChar) || (e.KeyChar.ToString().Equals(cc.NumberFormat.NumberDecimalSeparator) && !textBoxCantidad.Text.Contains(".")));

            /*if (char.IsNumber(e.KeyChar) || char.IsControl(e.KeyChar) || (e.KeyChar.ToString() == cc.NumberFormat.NumberDecimalSeparator && !textBoxCantidad.Text.Contains(".")))
            {
        ...
        private void buttonEditar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                //var result = santaDBContext.productos.AsNoTracking().SingleOrDefault(b => b.id_producto == textBoxCodigo.Text.ToUpper().Trim());

                //if (result != null && !textBoxCodigo.Text.ToUpper().Trim().Equals(codigo.ToUpper().Trim()))
                //{
                ...
                //        bool updatedCodeBar = false;

                //        if (result == null)
                //        {
                //            /*result = santaDBContext.productos.AsNoTracking().SingleOrDefault(b => b.id_producto == codigo);
                //            updatedCodeBar = true;*/

                //            updatedCodeBar = true;
                //            result = new producto();
                //            result.id_producto = textBoxCodigo.Text.ToUpper().Trim();

                //            santaDBContext.Entry(santaDBContext.productos.AsNoTracking().SingleOrDefault(b => b.id_producto == codigo)).State = System.Data.Entity.EntityState.Deleted;

                //            santaDBContext.SaveChanges();

                //        }

                //        result.nombre = textBoxNombre.Text.Trim();
                //        result.marca = textBoxMarca.Text.Trim();
                //        result.categoria = comboBoxCategoria.Text.Trim();
                //        result.cantidad = double.Parse(textBoxCantidad.Text);
                //        result.precio = decimal.Round(decimal.Parse(textBoxPrecio.Text), 2);
                //        result.fechaultact = DateTime.Now;

                //        santaDBContext.Entry(result).State = updatedCodeBar ? System.Data.Entity.EntityState.Added : System.Data.Entity.EntityState.Modified;
                         ...
                         + "\nMarca: " + textBoxMarca.Text + "\nCategoría: " + comboBoxCategoria.Text + "\nCantidad: " + textBoxCantidad.Text + "\nPrecio: " + textBoxPrecio.Text, "Pregunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        try
                        {
                                                                         ...
                                                                         "WHERE  id_producto = @id", openCon, transaction))
                            {
                                ...
                                transaction.Commit();
                                MessageBox.Show($"'{textBoxNombre.Text.Trim()}' ha sido editado correctamente", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                editado = true;
                                Dispose();
                            }
                        }
